---
name: operator_request_notify.py
slug: operator-request-notify-py
type: file
sources:
...
Orchestrates Telegram notifications for operator requests and enforces deterministic resolution rules per request type (document-procurement, credential, legal/financial decisions, expenditure-approval, external-action-authorization). Persists dedup state keyed on content fingerprint, retries on Telegram failure, and writes audit logs.
