...
links:
  - to: snapog-worker
    relation: part_of
    description: Served by the same Worker; landing page embeds live OG preview.
generator:
  version: 1
covers:
  - symbol: layout
...
Server-side HTML template module rendering the public landing page and dashboard with the 'Carbon Terminal' aesthetic, no client-side framework. Threads the host argument into code examples and meta tags to build absolute URLs, and embeds a live OG preview hitting the actual /og endpoint.

## Related

- part of [[snapog-worker]] — Served by the same Worker; landing page embeds live OG preview.
