---
name: operator_decision_panel_format
slug: operator-decision-panel-format
type: concept
sources:
...
links:
  - to: operator-request-notify
    relation: produces
    description: >-
      The panel writes the decision file that operator_request_notify.py
      consumes to mark requests REFUSED and audit-log them.
generator:
  version: 1
covers:
  - symbol: ps_quote
...
The operator-decision panel (dashboard/server.py) writes operator-decisions.md with a short single-line REFUSE head followed by verbatim multi-line reasoning, so numbered points survive as separate lines. This format is a contract: operator_request_notify.py parses it, and the refusal-format regression test pins that the head stays bounded and the body stays verbatim.

## Related

- produces [[operator-request-notify]] — The panel writes the decision file that operator_request_notify.py consumes to mark requests REFUSED and audit-log them.
