...
A narrow lint for the fatal set -e pattern: a '[ test ] && action' list used as a function's last command or immediately before a bare return propagates a false test's exit status 1 and kills an unguarded caller (root cause of APP-240). The scan deliberately skips done/fi/esac terminators (set -e exempts the left operand of AND-OR lists) and lines ending in || true / || : to avoid false positives on the safe redirect-fed loop form in docker-entrypoint.sh's runtime.env parser.
