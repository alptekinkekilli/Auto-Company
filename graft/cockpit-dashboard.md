...
sources_digest: 5894a7878160451879356a54940d38fff67bebd2abb27f1e8e4c4d68800a87da
links:
  - to: dashboard-server
    relation: uses
    description: >-
      Polls /api/status, /api/hold, /api/directive, /api/ideas, /api/tool-usage
      served by the dashboard server.
  - to: dashboard-server
    relation: part_of
    description: Frontend half of the cockpit dashboard service.
  - to: ops-audit-analytics-scripts
    relation: uses
    description: >-
      Consumes the durable ledgers (tool-usage-history.ndjson, etc.) produced by
      the audit scripts.
generator:
  version: 1
covers:
...
Browser-side SPA controller plus its stdlib-only Sentry reporter. Polls /api/status on a timer, renders live state cards (guardian, daemon, loop, autostart, graft freshness), cost/usage panels, directive editor, settings form, log/consensus views. All state comes from backend REST endpoints; no external libraries. Hand-rolled markdown rendering; consensus panel refuses to overwrite while operator reads full file; ccusage fails open; tool-usage rendered as real table because Rajdhani font is proportional.

## Related

- uses [[dashboard-server]] — Polls /api/status, /api/hold, /api/directive, /api/ideas, /api/tool-usage served by the dashboard server.
- part of [[dashboard-server]] — Frontend half of the cockpit dashboard service.
- uses [[ops-audit-analytics-scripts]] — Consumes the durable ledgers (tool-usage-history.ndjson, etc.) produced by the audit scripts.
