...
Browser-side SPA controller plus its stdlib-only Sentry reporter. Polls /api/status on a timer, renders live state cards (guardian, daemon, loop, autostart, graft freshness), cost/usage panels, directive editor, settings form, and log/consensus views. All state comes from backend REST endpoints; no external libraries. The consensus panel refuses to overwrite while the operator is reading the full file; ccusage fails open; tool-usage is a real table because the proportional Rajdhani font makes character-padding impossible.
