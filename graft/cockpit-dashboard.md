...
sources_digest: 4f84f82cbb29afe692f2a3dcecb684ed19de2d4806935b70d73439dba041967f
links:
  - to: budget-spend-accounting
    relation: uses
    description: >-
      Dashboard displays budget gates and spend windows anchored on ccusage
      blockStart.
  - to: dashboard-server
    relation: uses
    description: >-
      Polls /api/status and calls /api/hold, /api/directive, /api/ideas,
      /api/tool-usage; sentry_client POSTs exceptions to Sentry's legacy Store
      API.
  - to: escalation-operator-requests
    relation: produces
    description: >-
      The operator-decision panel writes operator-decisions.md that
      operator_request_notify.py parses.
generator:
  version: 1
covers:
  - symbol: escapeHtml
...
Browser-side SPA controller plus its stdlib-only Sentry reporter. Polls /api/status on a timer and renders live state cards, cost/usage panels, directive editor, settings form, and log/consensus views. All state comes from backend REST endpoints; no external libraries. The consensus panel refuses to overwrite while the operator is reading the full file, and the ccusage block fails open.

## Related

- uses [[budget-spend-accounting]] — Dashboard displays budget gates and spend windows anchored on ccusage blockStart.
- uses [[dashboard-server]] — Polls /api/status and calls /api/hold, /api/directive, /api/ideas, /api/tool-usage; sentry_client POSTs exceptions to Sentry's legacy Store API.
- produces [[escalation-operator-requests]] — The operator-decision panel writes operator-decisions.md that operator_request_notify.py parses.
