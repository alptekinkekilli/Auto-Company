...
links:
  - to: directive-writer
    ...
    description: >-
      ...
      directive_writer.py's lock/gate/atomic-rename pipeline.
  - to: operator-request-ledger
    relation: uses
    description: >-
      server.py reads operator-requests.md and appends decisions to
      operator-decisions.md in a layout load-bearing for
...
Browser-side SPA controller plus the ThreadingHTTPServer backend that monitors and controls the autonomous loop. Polls /api/status, renders live state cards, cost/usage panels, directive editor, settings, and log/consensus views; the server abstracts host platforms (WSL/macOS/Linux), reads/writes state files, and routes directive writes through the deterministic directive_writer.py. UI strings are mixed Turkish/English; all state comes from REST endpoints with no external libraries.

## Related

- uses [[directive-writer]] — server.py routes all human-directive.md writes through directive_writer.py's lock/gate/atomic-rename pipeline.
- uses [[operator-request-ledger]] — server.py reads operator-requests.md and appends decisions to operator-decisions.md in a layout load-bearing for operator_request_notify.py's regex window parsing.
