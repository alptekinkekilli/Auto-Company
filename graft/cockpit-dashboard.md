...
links:
  - to: cockpit-server
    ...
    description: >-
      ...
      /api/tool-usage; the server is the sole data source.
  - to: directive-writer
    relation: uses
    description: >-
      submitDirective's allowPending retry flow overrides a live PENDING
      directive, which the server routes through directive_writer.py's gate.
generator:
  version: 1
covers:
  - symbol: escapeHtml
...
Browser-side SPA controller plus its stdlib-only Sentry reporter. Polls /api/status on a timer and renders live state cards, cost/usage panels, directive editor, settings form, and log/consensus views. All state comes from backend REST endpoints; no external libraries. The consensus panel refuses to overwrite while the operator is reading the full file, and the ccusage block fails open when unavailable.

## Related

- uses [[cockpit-server]] — Polls /api/status and calls /api/hold, /api/directive, /api/ideas, /api/tool-usage; the server is the sole data source.
- uses [[directive-writer]] — submitDirective's allowPending retry flow overrides a live PENDING directive, which the server routes through directive_writer.py's gate.
