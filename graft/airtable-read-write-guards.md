...
links:
  - to: web-research-cost-model
    relation: uses
    description: >-
      Airtable dumps are the largest context source, motivating the all-tools
      cost scoring.
generator:
  version: 1
covers:
...
Wrappers that gate Airtable reads and writes to control context cost and data integrity. Reads refuse unscoped queries (exit 2), cap --all-fields and --max-records (200), and cap pageSize at 100. Writes validate single records: unknown fields refused unless --force, clearing non-empty fields needs --allow-clear, substantial value replacement needs --replace, and refusal messages name the missing flag.

## Related

- uses [[web-research-cost-model]] — Airtable dumps are the largest context source, motivating the all-tools cost scoring.
