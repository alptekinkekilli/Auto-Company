---
name: G4 Identity Attribution
slug: g4-identity-attribution
type: system
sources:
...
links:
  - to: airtable-read-write-wrappers
    relation: uses
    description: g4-check.py reads registry bridge records via air_get/air_list.
  - to: outreach-eligibility-brake
    relation: uses
    description: >-
      send-gate.py imports g4-check.py to re-derive G4 live before allowing
      outreach.
generator:
  version: 1
covers:
...
The evidence-based G4 identity-attribution verdict for Turkish firms, testing a row's claim against live evidence rather than trusting self-declared 'G4 PASS'. g4-check.py requires first-party contact plus an address/registry-number/Profile-bridge anchor; site-contact-evidence.py escalates through rendered DOM, raw HTML, and JS bundles, treating a no-render fetch as inconclusive, never negative. send-gate.py re-derives G4 live and requires an exact firm-name match.

## Related

- uses [[airtable-read-write-wrappers]] — g4-check.py reads registry bridge records via air_get/air_list.
- uses [[outreach-eligibility-brake]] — send-gate.py imports g4-check.py to re-derive G4 live before allowing outreach.
