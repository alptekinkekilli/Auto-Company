---
name: Outreach & send gating
slug: outreach-send-gating
type: system
sources:
...
links:
  - to: airtable-access-layer
    relation: uses
    description: reads Outreach rows and bridge rows
generator:
  version: 1
covers:
  - symbol: api_key
...
Fail-closed refusal policy for outbound outreach: daily/total caps, duplicate detection, opt-out, G4 verification, body-leak scanner for internal markers, exact normalized firm-name matching, and phase-check scoping to first-contact sends. Also the reply-watch outcome classifier (reply/bounce/silence, 72h threshold).

## Related

- uses [[airtable-access-layer]] — reads Outreach rows and bridge rows
