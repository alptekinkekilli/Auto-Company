---
name: Registry & G4 compliance tooling
slug: registry-g4-compliance-tooling
type: system
sources:
...
links:
  - to: state-snapshot-probe
    relation: uses
    description: >-
      The snapshot hashes the Wowcar source directory that these tools operate
      on.
generator:
  version: 1
covers:
...
registry-archive.py archives stale dated sections from a candidate registry markdown while preserving a protected live span byte-identical, and g4-check.py matches register addresses/registry IDs/domains with Turkish dotted/dotless İ-ı folding and a coincidence guard. Both encode real production failure cases (RAYELSİS, ARKENOM, MAGİM) as test fixtures.

## Related

- uses [[state-snapshot-probe]] — The snapshot hashes the Wowcar source directory that these tools operate on.
