---
name: Browser/context extraction
slug: browser-context-extraction
type: system
sources:
...
links:
  - to: decision-content-hash-canonicalization
    relation: uses
    description: kik-decision-read.py uses decision_text_hash.py for the canonical hash.
generator:
  version: 1
covers:
  - symbol: Gateway
...
Collapses multi-step MCP browse-and-extract workflows into single invocations to cut context re-billing. browse-extract.py walks URLs in one background tab with server-side grep over rendered content (so SPAs are searched post-render) and is read-only by design; kik-decision-read.py fetches a KİK decision page in a single curl call with retries.

## Related

- uses [[decision-content-hash-canonicalization]] — kik-decision-read.py uses decision_text_hash.py for the canonical hash.
