...
sources_digest: ae39c5ffe28edb8632ce34d2acdb1630c24478a42c4b0e03b916fc1c4f855d76
links: []
generator:
  version: 1
covers:
...
Cross-cutting invariant: every state mutation uses temp-file + os.replace (or equivalent) with read-back verification, never in-place writes, so a crash mid-write never leaves a torn file. Manifestations: directive_writer atomic_write, jcode-mcp-config atomic write with freshness stamp, idle-skip-note tempfile.mkstemp+os.replace, registry-archive compare-and-swap on mtime, cost-audit .tmp+os.replace, operator_request_notify atomic state writes with corruption backup.
