---
name: Outreach eligibility & G4 verification
slug: outreach-eligibility-g4-verification
type: system
sources:
...
links:
  - to: airtable-ops-wrappers
    relation: uses
    description: >-
      Reads Registry Bridge and Outreach tables via the scoped Airtable
      wrappers.
  - to: operator-escalation-notification
    relation: produces
    description: >-
      G4/attribution outcomes feed the operator-request and reply-watch
      notifications.
generator:
  version: 1
covers:
...
The fail-closed gatekeeping layer for autonomous outreach. send-gate.py answers 'may this firm be emailed now?' with cheapest-first checks (caps bind on messages not firms, opt-out, HOLD markers, body-leak scan) and REFUSEs on any incomplete check. g4-check.py re-derives the G4 identity-attribution verdict from live site/registry evidence rather than trusting a row's self-declared PASS, reporting CLAIMED_PASS_UNVERIFIED instead of downgrading. site-contact-evidence.py finds a firm's published contact email, treating a non-rendered fetch as inconclusive, never negative.

## Related

- uses [[airtable-ops-wrappers]] — Reads Registry Bridge and Outreach tables via the scoped Airtable wrappers.
- produces [[operator-escalation-notification]] — G4/attribution outcomes feed the operator-request and reply-watch notifications.
