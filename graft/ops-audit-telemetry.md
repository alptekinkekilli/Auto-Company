---
name: Ops audit & telemetry
slug: ops-audit-telemetry
type: system
sources:
...
sources_digest: 1303a81adf38b17a4065abeba993ce9a4ef3137ca7a5bdb4806a156a1ac48ea8
links:
  - to: auto-loop-core
    relation: produces
    description: Consumes the cycle-ndjson and jcode log streams that the loop writes.
generator:
  version: 1
covers:
  - symbol: externals
...
Per-cycle and per-session audit scripts that measure cost, tool usage, turn economics, and web-research residual cost from NDJSON/log streams. All are read-only, always exit 0, and use only the standard library; they power the cockpit's analytics panels and post-cycle hooks.

## Related

- produces [[auto-loop-core]] — Consumes the cycle-ndjson and jcode log streams that the loop writes.
