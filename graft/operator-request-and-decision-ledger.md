---
name: Operator request and decision ledger
slug: operator-request-and-decision-ledger
type: system
sources:
...
links:
  - to: auto-loop-core-engine
    relation: uses
    description: >-
      auto-loop.sh runs operator_request_notify.py as the OPREQ ledger step
...
The memories/ ledger of operator requests, decisions, and consensus, plus the scripts that notify, resolve, and route them: operator_request_notify.py (Telegram notifications, deterministic resolution rules), operator-action-router.py (priority hold>opreq>directive), and the refusal-format handling.

## Related

- uses [[auto-loop-core-engine]] — auto-loop.sh runs operator_request_notify.py as the OPREQ ledger step before sleeping on idle-skip.
