...
links:
  ...
  - to: telegram-notification
    relation: uses
    description: Sends OPREQ notifications through the shared telegram-notify.sh channel.
generator:
  version: 1
covers:
...
The deterministic operator-escalation gate for the company knowledge base: the only component allowed to decide whether an OPREQ block is genuinely new or changed (via SHA-256 material_hash), send Telegram notifications (marking delivery only on ok:true), flip OPEN→RESOLVED through type-specific checks, and regenerate the ## Awaiting Operator projection in consensus.md. Always exits 0 so it never breaks the calling loop; resolution verification rejects free-text self-claims, requiring a human-directive.md reference plus a verifiable artifact.

## Related

- uses [[human-directive-writer]] — Resolution verification requires a human-directive.md reference.
- uses [[telegram-notification]] — Sends OPREQ notifications through the shared telegram-notify.sh channel.
