---
name: Registry merge tool
slug: registry-merge-tool
type: file
sources:
...
links:
  - to: opportunity-analyst-pipeline
    relation: part_of
    description: Invoked as pass 2 of the analyst pipeline.
generator:
  version: 1
covers:
...
Deterministic PASS-2 merge that surgically replaces only the live decision-state span of candidate-registry.md, leaving the ~180KB append-only journal byte-identical. Enforces invariants: no candidate ID disappears, no duplicate axes, active-candidate identity not rewritable. Fail-closed with read-back verification.

## Related

- part of [[opportunity-analyst-pipeline]] — Invoked as pass 2 of the analyst pipeline.
