...
links:
  - to: cockpit-dashboard
    relation: part_of
    description: Used by dashboard/server.py for error reporting.
generator:
  version: 1
covers:
  - symbol: _parse_dsn
...
A minimal stdlib-only Sentry error reporter (no sentry-sdk, since the container has no pip deps). Parses the DSN at import time, builds a Sentry v7 event with stacktrace, and POSTs to the legacy Store API with a 3s timeout. Best-effort: any failure is caught and logged, never raised, so monitoring cannot crash the dashboard.

## Related

- part of [[cockpit-dashboard]] — Used by dashboard/server.py for error reporting.
