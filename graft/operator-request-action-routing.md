---
name: Operator request & action routing
slug: operator-request-action-routing
type: system
sources:
...
links:
  - to: auto-loop-core
    relation: uses
    description: >-
      The loop runs the OPREQ ledger step before sleeping and routes actions
...
The ledger and notification machinery for operator requests: operator_request_notify.py orchestrates Telegram notifications with deterministic per-type resolution rules (evidence checksum, credential PASS log, structured decision/authorization lines), and operator-action-router.py prioritizes hold > opreq > directive with staleness floors and dedup.

## Related

- uses [[auto-loop-core]] — The loop runs the OPREQ ledger step before sleeping and routes actions based on these modules.
