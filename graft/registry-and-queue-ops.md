---
name: registry-and-queue-ops
slug: registry-and-queue-ops
type: system
sources:
...
links:
  - to: airtable-read-write-guards
    relation: uses
    description: >-
      The queue watcher reads Airtable records (stubbed in tests) to count
      pending registry requests and attribution-held firms.
generator:
  version: 1
covers:
  - symbol: die
...
Python tools that maintain the candidate registry markdown and watch its queue: registry-archive.py archives stale dated sections while preserving a protected live span byte-identically, and registry-queue-watch.py alerts on queue backlogs while distinguishing company gaps from operator bottlenecks. Both are regression-tested with synthetic fixtures and dry-run modes.

## Related

- uses [[airtable-read-write-guards]] — The queue watcher reads Airtable records (stubbed in tests) to count pending registry requests and attribution-held firms.
