---
name: MCP Configuration & Boot Probe
slug: mcp-configuration-boot-probe
type: system
sources:
...
links:
  - to: jcode-event-stream-utilities
    relation: uses
    description: >-
      The probe's generated config feeds the jcode runs whose ndjson streams the
      utilities consume.
generator:
  version: 1
covers:
...
Generates and verifies jcode's MCP server configuration. jcode-mcp-config.py rewrites project .mcp.json into ~/.jcode/mcp.json, wrapping HTTP/SSE servers in the mcp-remote stdio bridge, expanding ${VAR} env vars, and writing a freshness stamp. jcode-mcp-probe.py then speaks JSON-RPC 2.0 over stdio to each server, enforcing five fail-closed gates (server-set match, handshake timeout, destructive-tool match, JCODE_TOOLS_DENY coverage, and a mandatory read-only tools/call per server — tools/list alone proved insufficient when Context7 passed boot for days without ever being called).

## Related

- uses [[jcode-event-stream-utilities]] — The probe's generated config feeds the jcode runs whose ndjson streams the utilities consume.
