...
A SessionStart hook that conditionally triggers a paid deep build of graft cards only when git history shows genuine staleness (commits-behind and age double threshold). Deliberately fail-open, non-blocking (detached via start_new_session), with the Together API key never touching this script — it lives in graft-build.sh.
