...
Canonical SQL for Weekly Active Producers (WAP): counts distinct api_key_id values that generated at least one non-cached OG image (cache_hit=0) within a window. Cache hits are excluded because they represent edge-served static responses with near-zero marginal cost and no signal of new demand. Days with zero traffic produce no rows, so zero-filling must happen client-side.
