...
sources_digest: 39bfad933af62fb311e39928cec7513a90676ca29c3c1679ecaf1e6a69a4b165
links:
  - to: outreach-eligibility-brake
    relation: implements
    description: send-gate.py is the canonical fail-closed brake.
generator:
  version: 1
covers:
  - symbol: ServerError
...
A pervasive design principle: any check that cannot complete is a REFUSE/error, never an ALLOW/silent success. Appears in send-gate.py (any incomplete check refuses), jcode-mcp-probe.py (five fail-closed gates), extract-axis-evidence.py (fails on any unreadable file or count mismatch), registry-archive.py (any invariant violation writes nothing), and directive-rule-sweep.py (exits 3 if live directive unreadable to avoid false completeness).

## Related

- implements [[outreach-eligibility-brake]] — send-gate.py is the canonical fail-closed brake.
