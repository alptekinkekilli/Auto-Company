...
CLI harness collapsing multi-step MCP browse-and-extract into one bash invocation, cutting context re-billing from ~5-9 turns to one. Walks URLs in one background tab via BrowserOS MCP gateway (JSON-RPC over HTTP with session-ID persistence, SSE framing). Read-only by design, closes tab even on error unless --keep-tab, timed-out wait reported as waited=timeout so zero grep matches are inconclusive.
