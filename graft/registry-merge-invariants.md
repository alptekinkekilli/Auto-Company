...
links:
  - to: opportunity-analyst
    relation: part_of
    description: merge_registry.py is the PASS-2 tool used by the analyst.
generator:
  version: 1
covers:
...
The deterministic PASS-2 merge contract for candidate-registry.md: only the live decision-state span (between '## Selected' and '## Exhausted patterns / lessons') may be replaced, leaving the ~180KB append-only journal byte-identical. Invariants: no candidate ID may disappear, no duplicate axes, and the identity (axis) of active candidates cannot be rewritten — only verdict changes are allowed. Fail-closed with read-back verification and restoration on failure.

## Related

- part of [[opportunity-analyst]] — merge_registry.py is the PASS-2 tool used by the analyst.
