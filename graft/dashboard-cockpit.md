...
links:
  - to: budget-spend-accounting
    relation: uses
    description: >-
      The dashboard displays budget gates and spend windows parsed from
      auto-loop.log and ccusage.
  - to: escalation-operator-requests
    relation: produces
    description: >-
      The decision panel writes operator-decisions.md in the format
      operator_request_notify parses.
generator:
  version: 1
covers:
  - symbol: ps_quote
...
dashboard/server.py serves the operator cockpit: status parsing, action dispatch, engine runtime state from the persistent auto-loop.log (latest boot, escalated cycles, legacy tier lines), spend window anchoring on ccusage blockStart, tail truncation, weekly cost split, and the operator-decision panel that writes the REFUSE head-line format. It imports a sibling sentry_client, which tests must account for when loading via importlib.

## Related

- uses [[budget-spend-accounting]] — The dashboard displays budget gates and spend windows parsed from auto-loop.log and ccusage.
- produces [[escalation-operator-requests]] — The decision panel writes operator-decisions.md in the format operator_request_notify parses.
