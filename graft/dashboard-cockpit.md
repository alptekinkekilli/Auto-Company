...
links:
  - to: operator-request-decision-resolution
    relation: produces
    description: Writes the decision/refusal format that operator_request_notify.py parses
generator:
  version: 1
covers:
  - symbol: ps_quote
...
The web dashboard that surfaces loop status, engine runtime state, budget display, cost summary, and log tailing, plus the operator-decision panel that writes refusal/decision formats. Parses the persistent auto-loop.log to reflect the latest boot including escalated cycles and legacy tier lines.

## Related

- produces [[operator-request-decision-resolution]] — Writes the decision/refusal format that operator_request_notify.py parses
