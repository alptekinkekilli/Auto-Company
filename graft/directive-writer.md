...
links:
  - to: cockpit-dashboard-server
    relation: implements
    description: Provides the write path the server routes directive writes through.
  - to: promotion-gate
    relation: uses
    description: >-
      promote_directive.py calls directive_writer to safely apply the new
      directive.
generator:
  version: 1
covers:
...
Sole writer for memories/human-directive.md. Fail-closed: in-flight PENDING never clobbered without --allow-pending; body immutable after acceptance (only Status changes via compare-and-swap on body hash). Every write goes through lock→read→gate→backup→atomic rename→verify→audit→notify.

## Related

- implements [[cockpit-dashboard-server]] — Provides the write path the server routes directive writes through.
- uses [[promotion-gate]] — promote_directive.py calls directive_writer to safely apply the new directive.
