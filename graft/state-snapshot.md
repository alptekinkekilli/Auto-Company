...
Parses human-directive.md and operator-requests.md into a local state snapshot with DELTA change-detection; errored fields are excluded from the next DELTA and a missing ledger prints an error but still exits 0. All fields are local since a 2026-08-24 re-charter (bridge/send/reply fields retired).
