---
name: Turn-Economy & Compliance Checks
slug: turn-economy-compliance-checks
type: system
sources:
...
sources_digest: 8b4a4e8f7b2907066d590ed9c01ecbfc20c2b994ca346e801f96b3a1d4aa652f
links:
  - to: telegram-notification-channel
    relation: uses
    description: bloat-trend.py alerts via telegram-notify.sh on regression or target-met.
generator:
  version: 1
covers:
  - symbol: ingest
...
Trend and compliance watchers over per-cycle logs. bloat-trend.py folds audit lines into durable NDJSON history and compares sliding windows, recomputing stored verdicts with current thresholds (thresholds changed 2026-08-02) so comparisons measure cycles not the ruler. context7-check.py detects code importing external libraries without first calling Context7, parsing JSON lines rather than regexing payloads to avoid silent false negatives. g4-check.py (also here) re-derives G4 from live evidence.

## Related

- uses [[telegram-notification-channel]] — bloat-trend.py alerts via telegram-notify.sh on regression or target-met.
