...
links:
  - to: airtable-access-wrappers
    relation: uses
    description: registry-queue-watch.py reads Airtable records
generator:
  version: 1
covers:
  - symbol: die
...
Tools that archive stale dated sections from a candidate registry markdown (preserving a protected live span byte-identical) and watch the registry queue for bottlenecks, distinguishing an empty bridge queue with many attribution-Held firms as a company gap rather than an operator gap. The watcher fires only above threshold and respects cooldown via a state file.

## Related

- uses [[airtable-access-wrappers]] — registry-queue-watch.py reads Airtable records
