---
name: HeadInspect Grading
slug: headinspect-grading
type: file
sources:
...
links:
  - to: headinspect-worker
    relation: part_of
    description: Imported by index.ts for categorization and grading.
generator:
  version: 1
covers:
  - symbol: Category
...
Pure, dependency-free logic for categorizing response headers and computing a security letter grade with weighted scoring and hard caps. Deterministic category ordering, a regex best-effort split for multi-cookie set-cookie values (due to Cloudflare's joining behavior), and a grade capped at C when no enforced CSP is present.

## Related

- part of [[headinspect-worker]] — Imported by index.ts for categorization and grading.
