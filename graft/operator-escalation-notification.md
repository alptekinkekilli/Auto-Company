...
sources_digest: 3588201e7e84d699269275cc8946bbe92b876cb372147e259d1e5ec8d68291ef
links:
  - to: directive-rule-compliance-watchers
    relation: uses
    description: >-
      operator-action-router.py reads the PENDING directive status that
      directive-staleness-watch.py also monitors.
  - to: telegram-notification-channel
    relation: uses
    description: Both send notifications through telegram-notify.sh.
generator:
  version: 1
covers:
...
Deterministic operator-facing gates and digests. operator_request_notify.py is the only component allowed to decide OPREQ novelty (SHA-256 material_hash), send Telegram, flip OPEN to RESOLVED via type-specific checks (verify_document_procurement enforces checksum-matched files confined to memories/operator-evidence/ with path-traversal protection), and regenerate the Awaiting Operator projection; it always exits 0 and redacts secrets shape-based. operator-action-router.py consolidates LOOP_HOLD, open OPREQs, and PENDING directive into one 'what needs YOU' digest, hashing the open set's stable identity to avoid re-alerts.

## Related

- uses [[directive-rule-compliance-watchers]] — operator-action-router.py reads the PENDING directive status that directive-staleness-watch.py also monitors.
- uses [[telegram-notification-channel]] — Both send notifications through telegram-notify.sh.
