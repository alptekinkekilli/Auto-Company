...
links:
  - to: jcode-event-stream-utilities
    relation: uses
    description: >-
      cost-audit and bloat-trend parse auto-loop.log and jcode logs for
      telemetry and turn-audit lines.
  - to: operator-escalation-notification
    relation: uses
    description: bloat-trend sends Telegram alerts via telegram-notify.sh.
generator:
  version: 1
covers:
  - symbol: ingest
...
Deterministic cost/budget reporting that parses on-disk logs: daily cost-audit, budget-calibration report, bloat-trend watcher, and operator-usage push. All deliberately never estimate, judge, or modify thresholds — they only report measured numbers, splitting pre/post policy cutovers.

## Related

- uses [[jcode-event-stream-utilities]] — cost-audit and bloat-trend parse auto-loop.log and jcode logs for telemetry and turn-audit lines.
- uses [[operator-escalation-notification]] — bloat-trend sends Telegram alerts via telegram-notify.sh.
