...
links:
  - to: airtable-access-layer
    relation: uses
    description: send-gate and g4-check read Airtable via the air() wrapper.
  - to: operator-escalation-notification
    relation: uses
    description: >-
      g4-check and site-contact-evidence feed evidence that may trigger operator
      escalation.
generator:
  version: 1
covers:
...
send-gate is the fail-closed brake on autonomous outreach: it answers 'may this firm be emailed now?' and exits without sending. Checks run cheapest-first (daily/total caps binding on messages, never-send-twice, opt-out, Qualified status, HOLD markers, body-leak scan, Exclusion ground limits) and any check that cannot complete is a REFUSE, never an ALLOW. G4 is re-derived live by importing g4-check.py.

## Related

- uses [[airtable-access-layer]] — send-gate and g4-check read Airtable via the air() wrapper.
- uses [[operator-escalation-notification]] — g4-check and site-contact-evidence feed evidence that may trigger operator escalation.
