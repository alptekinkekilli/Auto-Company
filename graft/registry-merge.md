...
Deterministic PASS-2 merge tool that surgically replaces only the 'live decision state' span of candidate-registry.md (between ## Selected and ## Exhausted patterns / lessons headings) with a model-proposed replacement, leaving the ~180KB append-only historical journal byte-identical. Enforces invariants: no candidate ID may disappear, no duplicate axes, identity (axis) of active candidates cannot be rewritten — only verdict changes allowed. Fail-closed exit 0 with BLOCKED:/MERGED: stdout, read-back verification and restoration on failure. Never trusts model instruction-following — proves prefix/suffix byte-identity via exact string slicing and hashes, logging hashes on every outcome.
