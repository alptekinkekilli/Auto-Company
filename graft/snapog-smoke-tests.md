...
links:
  - to: snapog-cost-alerting
    relation: validates
    description: Dry-runs the alert cron handler against a local dev server.
...
One-off operational validation scripts for SnapOG: smoke test of /health and /og, cache-cap test verifying X-Cache: BYPASSED after MONTHLY_CACHE_KEY_CAP, and alerts dry-run seeding 500 cache-miss rows to trip the 14-day hit-rate threshold.

## Related

- validates [[snapog-cost-alerting]] — Dry-runs the alert cron handler against a local dev server.
