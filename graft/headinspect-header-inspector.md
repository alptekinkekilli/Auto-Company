...
Cloudflare Worker that fetches a user-supplied URL and reports response headers with categorization and a security letter grade. Strict SSRF protections (rejects localhost, private IPs, cloud metadata), manual redirect following, 10s timeout, badge always 200 with edge caching.
