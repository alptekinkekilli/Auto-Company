...
sources_digest: 9d0a605d2827017229970922c770241ebe8fae6eb4e884058e6cc728945a9011
links:
  - to: auto-loop-core
    relation: uses
    description: >-
      auto-loop.sh runs operator_request_notify.py before sleeping in the
      idle-skip branch and consumes escalation directives.
  - to: ops-probe-audit-scripts
    relation: uses
    description: >-
      state-snapshot counts open OPREQ-* blocks in
...
The operator-request ledger and notification layer: operator_request_notify.py orchestrates Telegram notifications with content-fingerprint dedup and per-type resolution rules (document-procurement needs a checksum-matched evidence file, credential needs a PASS log with no secret-shaped tokens, decisions need a structured Decision/Authorization line in human-directive.md). operator-action-router.py prioritizes hold > opreq > directive with staleness floors and dedup.

## Related

- uses [[auto-loop-core]] — auto-loop.sh runs operator_request_notify.py before sleeping in the idle-skip branch and consumes escalation directives.
- uses [[ops-probe-audit-scripts]] — state-snapshot counts open OPREQ-* blocks in memories/operator-requests.md.
