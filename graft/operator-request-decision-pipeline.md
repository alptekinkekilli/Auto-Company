---
name: operator request & decision pipeline
slug: operator-request-decision-pipeline
type: system
sources:
...
links:
  - to: auto-loop-core
    relation: uses
    description: >-
      auto-loop.sh calls operator_request_notify.py as an OPREQ ledger step
      before sleeping on idle-skip.
generator:
  version: 1
covers:
...
Orchestration of operator requests and human decisions: operator_request_notify.py sends Telegram notifications with content-fingerprint dedup and enforces per-type resolution rules (checksum-matched evidence files, PASS logs, structured Decision/Authorization lines), while operator-action-router.py prioritizes hold > opreq > directive and persists state.

## Related

- uses [[auto-loop-core]] — auto-loop.sh calls operator_request_notify.py as an OPREQ ledger step before sleeping on idle-skip.
