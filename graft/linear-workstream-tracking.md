---
name: Linear workstream tracking
slug: linear-workstream-tracking
type: system
sources:
...
links:
  - to: auto-company-loop-core
    relation: part_of
    description: The loop's findings are recorded into Linear tracks.
generator:
  version: 1
covers:
  - symbol: key
...
Enforces a workstream discipline for Linear: appends checklist items to one long-lived 'track' issue per workstream instead of opening a new issue per finding. cmd_new only creates a real issue if --why matches one of three hardcoded justifications.

## Related

- part of [[auto-company-loop-core]] — The loop's findings are recorded into Linear tracks.
