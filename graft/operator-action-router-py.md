---
name: operator-action-router.py
slug: operator-action-router-py
type: file
sources:
...
Routes operator actions with priority ordering (LOOP_HOLD > operator-requests > human-directive), applies staleness floors, dedups within a repeat window, and renders a Turkish digest. Fail-soft when memories/ is missing; persists state.

## Related

- uses [[operator-request-notify-py]]
