...
links:
  - to: snapog-worker
    relation: implements
    description: Implements the /og image generation the worker routes to.
generator:
  version: 1
covers:
...
OG image generation module: generateOGImage builds a VNode tree via buildElement and wraps it in a workers-og ImageResponse at 1200×630. buildCacheKey produces a deterministic SHA-256 hex digest by JSON-stringifying params (with watermark merged) after sorting keys alphabetically, avoiding hash instability from object key ordering. Templates dispatch on template field to default/blog/article builders with theme-aware palettes and dynamic font sizing.

## Related

- implements [[snapog-worker]] — Implements the /og image generation the worker routes to.
