---
name: Wowcar revenue relabel acceptance harness
slug: wowcar-revenue-relabel-acceptance-harness
type: file
sources:
...
links:
  - to: ops-probe-audit-scripts
    relation: part_of
    description: >-
      Lives in the same ops scripts directory and follows the same fail-closed,
...
A fail-closed acceptance harness that pins the live Wowcar repository as an immutable baseline (SHA-256 manifest + canonical JSON root digest) and proves a candidate copy with eight approved vocabulary edits and six column-width adjustments produces byte-identical outputs. Hard-pins CPython, LibreOffice, and library versions, sets deterministic env (PYTHONHASHSEED=0, PYTHONDONTWRITEBYTECODE=1), and reverses edits to prove byte-equality.

## Related

- part of [[ops-probe-audit-scripts]] — Lives in the same ops scripts directory and follows the same fail-closed, deterministic-verification philosophy.
