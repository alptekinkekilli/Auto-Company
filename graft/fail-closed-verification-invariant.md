...
sources_digest: 80e9f1cf4eb40b527828a53caab00c8ff2748ebaf456ee25b8baa7a2da23828e
links:
  - to: outreach-eligibility-evidence
    relation: implements
    description: send-gate.py and site-contact-evidence.py embody this invariant.
generator:
  version: 1
covers:
  - symbol: key_phrases
...
A pervasive design rule: any check that cannot complete, or any evidence that cannot be obtained, must be treated as a failure/refusal, never as a silent pass. Manifested in send-gate.py (any incomplete check is REFUSE), site-contact-evidence.py (no rendered content is inconclusive, never negative), extract-axis-evidence.py (any unreadable file exits non-zero), registry-archive.py (any invariant violation writes nothing), and directive-rule-sweep.py (exits 3 on UNKNOWN to avoid false completeness).

## Related

- implements [[outreach-eligibility-evidence]] — send-gate.py and site-contact-evidence.py embody this invariant.
