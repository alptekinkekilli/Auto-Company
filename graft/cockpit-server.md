...
links:
  - to: cockpit-dashboard
    relation: produces
    description: Serves the SPA and all /api endpoints the dashboard consumes.
  - to: directive-writer
    relation: uses
    description: >-
  ...
  - to: operator-request-notify
    relation: produces
    description: >-
      Appends decisions to operator-decisions.md in a layout load-bearing for
      operator_request_notify.py's regex window parsing.
  - to: sentry-reporter
    relation: uses
    description: Reports errors via sentry_client.capture_exception.
generator:
  version: 1
covers:
  - symbol: ps_quote
...
ThreadingHTTPServer-based local web service that monitors and controls the autonomous loop across Windows (WSL), macOS, and Linux. Abstracts host via get_host_profile(), reads/writes state files (human-directive.md, operator-requests.md, operator-decisions.md, runtime.env), and routes directive writes through the deterministic directive_writer.py. Logs are read with a 256KB tail window because auto-loop.log grows across redeploys; decoding tries utf-8/utf-8-sig/gb18030/cp936 for Windows. SETTINGS_SPEC whitelist prevents persisting secrets.

## Related

- produces [[cockpit-dashboard]] — Serves the SPA and all /api endpoints the dashboard consumes.
- uses [[directive-writer]] — Routes all human-directive.md writes through scripts/core/directive_writer.py for locking and PENDING gating.
- produces [[operator-request-notify]] — Appends decisions to operator-decisions.md in a layout load-bearing for operator_request_notify.py's regex window parsing.
- uses [[sentry-reporter]] — Reports errors via sentry_client.capture_exception.
