...
links:
  - to: cockpit-dashboard
    relation: produces
    description: Serves the SPA and all /api endpoints the dashboard polls.
  - to: directive-writer
    relation: uses
    description: >-
      Routes all human-directive.md writes through
      scripts/core/directive_writer.py which locks, gates on in-flight PENDING
      directives, and refuses clobbering unless allow_pending is set.
  - to: sentry-reporter
    relation: uses
    description: Imports sentry_client.capture_exception for error reporting.
generator:
  version: 1
covers:
  - symbol: ps_quote
...
ThreadingHTTPServer-based local web service that monitors and controls the autonomous loop across Windows (WSL), macOS, and Linux. Abstracts host via get_host_profile() pairing each OS with a script runner and status parser. Reads/writes state files: human-directive.md (routed through the deterministic directive_writer.py), operator-requests.md ledger, operator-decisions.md (layout load-bearing for operator_request_notify.py's regex window parsing). SETTINGS_SPEC whitelist prevents persisting secrets to runtime.env. Logs read via 256KB tail window because auto-loop.log grows across redeploys; decoding tries utf-8/utf-8-sig/gb18030/cp936 for Windows. Analyst trigger is file-based (ANALYST_RUN_REQUEST) because the cockpit runs inside the app container and cannot start host-side cron containers.

## Related

- produces [[cockpit-dashboard]] — Serves the SPA and all /api endpoints the dashboard polls.
- uses [[directive-writer]] — Routes all human-directive.md writes through scripts/core/directive_writer.py which locks, gates on in-flight PENDING directives, and refuses clobbering unless allow_pending is set.
- uses [[sentry-reporter]] — Imports sentry_client.capture_exception for error reporting.
