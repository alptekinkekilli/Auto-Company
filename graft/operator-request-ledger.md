...
links:
  - to: cockpit-dashboard
    relation: part_of
    description: server.py reads the ledger and appends decisions.
generator:
  version: 1
covers:
  - symbol: ps_quote
...
The operator-request ledger (memories/operator-requests.md) and its decisions file (operator-decisions.md) form a load-bearing layout: server.py appends decisions in a layout that operator_request_notify.py's regex-based window parsing depends on. The dashboard reads the ledger and appends decisions, so the exact append format is a cross-file invariant.

## Related

- part of [[cockpit-dashboard]] — server.py reads the ledger and appends decisions.
