...
links:
  - to: cockpit-dashboard
    relation: produces
    description: Serves the /api/* endpoints the dashboard polls and renders.
  - to: directive-writer
    relation: uses
    description: >-
      Routes all human-directive.md writes through directive_writer.py, which
      locks, gates on in-flight PENDING directives, and refuses clobbering
      unless allow_pending is set.
  - to: sentry-reporter
    relation: uses
    description: Reports server errors through the external sentry_client library.
generator:
  version: 1
covers:
  - symbol: ps_quote
...
ThreadingHTTPServer-based local web service exposing REST endpoints for the cockpit. Abstracts host platforms (WSL/macOS/Linux) with paired script runners and status parsers, reads/writes the human-directive via directive_writer.py, appends operator decisions in a load-bearing layout, and enforces a SETTINGS_SPEC whitelist so the panel never persists secrets. Logs are read with a 256KB tail window because auto-loop.log grows across redeploys.

## Related

- produces [[cockpit-dashboard]] — Serves the /api/* endpoints the dashboard polls and renders.
- uses [[directive-writer]] — Routes all human-directive.md writes through directive_writer.py, which locks, gates on in-flight PENDING directives, and refuses clobbering unless allow_pending is set.
- uses [[sentry-reporter]] — Reports server errors through the external sentry_client library.
