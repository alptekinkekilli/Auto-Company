...
links:
  - to: airtable-access-layer
    relation: uses
    description: >-
      watcher reads pending registry requests and attribution-held firms from
      Airtable
generator:
  version: 1
covers:
...
Candidate registry tooling: archives stale dated sections while preserving a protected live span byte-identical, and a queue watcher that fires only above threshold with cooldown and distinguishes empty-bridge-queue-with-held-firms as a company gap, not an operator gap.

## Related

- uses [[airtable-access-layer]] — watcher reads pending registry requests and attribution-held firms from Airtable
