...
sources_digest: c64988d560261af876a888db8f766946d3bc95cb1cac9c600d84bd70e58efd56
links:
  - to: airtable-ops-wrappers
    relation: uses
    description: Loop writes spend ledgers and reads state through Airtable wrappers.
generator:
  version: 1
covers:
...
The central autonomous outreach loop (scripts/core/auto-loop.sh) plus its budget gates, cycle counter, cycle metadata extraction, escalation, idle-skip, and mixed-harness engine routing. Enforces monotonic cycle counters, fail-closed ccusage measurement, budget gates per APP-263, one-shot operator escalation, and per-cycle engine attribution.

## Related

- uses [[airtable-ops-wrappers]] — Loop writes spend ledgers and reads state through Airtable wrappers.
