---
name: Audit & telemetry tooling
slug: audit-telemetry-tooling
type: system
sources:
...
sources_digest: d58a092de357671c0d5e4a51ffe588dce6ea2493b12cdf0de8d6f9f4c4bb9a31
links:
  - to: auto-loop-core-auto-loop-sh
    relation: produces
    description: Consume cycle-ndjson event streams produced by the loop.
generator:
  version: 1
covers:
  - symbol: externals
...
Per-cycle and per-session auditors: tool-usage-audit.py (durable ledger, idempotent via filename+size+mtime), turn-audit.py (LLM turn economics, env-overridable verdicts), web-research-cost.py (residual cost = output_tokens × turns_after × cache rate), context7-check.py (external imports must have Context7 lookup). All stdlib-only, always exit 0.

## Related

- produces [[auto-loop-core-auto-loop-sh]] — Consume cycle-ndjson event streams produced by the loop.
