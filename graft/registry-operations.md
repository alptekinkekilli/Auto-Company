...
links:
  - to: airtable-read-write-guards
    relation: uses
    description: 'registry-queue-watch reads Airtable via urllib, stubbed in tests.'
generator:
  version: 1
covers:
  - symbol: die
...
Registry-archive.py (archives stale dated sections while preserving a protected live span byte-identical) and registry-queue-watch.py (fires only above threshold, respects cooldown, and distinguishes an empty bridge queue with many attribution-Held firms as a company gap, not an operator gap).

## Related

- uses [[airtable-read-write-guards]] — registry-queue-watch reads Airtable via urllib, stubbed in tests.
