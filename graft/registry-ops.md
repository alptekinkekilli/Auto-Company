...
links:
  - to: airtable
    relation: uses
    description: >-
      registry-queue-watch stubs Airtable via urllib.request.urlopen
      monkey-patching to inject fake pending/held/EKAP records.
generator:
  version: 1
covers:
  - symbol: die
...
The registry maintenance tools: registry-archive.py archives stale dated sections from a candidate registry markdown into a monthly archive file while preserving a protected live span (from '## Selected' through '## Exhausted patterns / lessons') byte-identical, never moving frozen-pattern sections inside the protected region, and is idempotent; registry-queue-watch.py (APP-277) fires only above threshold with cooldown persistence and distinguishes an empty bridge queue with many attribution-Held firms as a company gap (not an operator gap), detecting EKAP-only queues (a v1 blind spot).

## Related

- uses [[airtable]] — registry-queue-watch stubs Airtable via urllib.request.urlopen monkey-patching to inject fake pending/held/EKAP records.
