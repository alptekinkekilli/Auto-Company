---
name: Test extraction strategy
slug: test-extraction-strategy
type: concept
sources:
...
links:
  - to: auto-loop-core
    relation: validates
    description: This strategy is how the auto-loop core is validated.
generator:
  version: 1
covers: []
---
...
The dominant testing convention: bash tests extract the real function bodies from auto-loop.sh via awk/sed and drive them under production's exact shell options, stubbing external binaries (date, ccusage, jcode, timeout) to run offline and deterministically. This ensures tests exercise shipping code, not copies, and any change to extraction patterns breaks loudly rather than silently passing.

## Related

- validates [[auto-loop-core]] — This strategy is how the auto-loop core is validated.
