---
name: Context7 & browse extraction
slug: context7-browse-extraction
type: system
sources:
...
links:
  - to: mcp-configuration-key-security
    relation: uses
    description: browse-extract.py talks to the browseros MCP server
generator:
  version: 1
covers:
  - symbol: Gateway
...
Tools that audit AI cycles for external library imports (requiring a Context7 documentation lookup) and extract browser content via the MCP gateway, always closing the browser tab even on error paths. The context7 check must not fire on the project's own stdlib-only ops scripts to avoid false alarms that erode trust.

## Related

- uses [[mcp-configuration-key-security]] — browse-extract.py talks to the browseros MCP server
