...
links:
  - to: snapog-cost-alerting
    relation: uses
    description: Cron-triggered runCostAlertCheck from ./alerts.
generator:
  version: 1
covers:
  - symbol: layout
...
Cloudflare Worker generating Open Graph images on demand. Hashes API keys before storage, enforces monthly usage limits, caches to R2, and caps distinct cache keys per key per month (beyond cap: still renders but skips R2 put, returns X-Cache: BYPASSED).

## Related

- uses [[snapog-cost-alerting]] — Cron-triggered runCostAlertCheck from ./alerts.
