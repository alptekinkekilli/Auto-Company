...
links:
  - to: cockpit-dashboard
    relation: produces
    description: Serves the REST endpoints and state files that app.js renders.
  - to: directive-writer
    relation: uses
    description: >-
      Routes all human-directive.md writes through
      scripts/core/directive_writer.py to enforce the PENDING gate and body
      immutability.
  - to: sentry-reporter
    relation: uses
...
ThreadingHTTPServer-based local web service that monitors and controls the autonomous loop across Windows (WSL), macOS, and Linux. Abstracts host via get_host_profile() pairing each OS with a script runner and status parser. Reads/writes state files: human-directive.md (routed through the single deterministic directive_writer.py), operator-requests/decisions ledgers, and a SETTINGS_SPEC whitelist restricting which non-secret knobs can be written to runtime.env. Logs are read with a 256KB tail window because auto-loop.log grows across redeploys; decoding tries utf-8, utf-8-sig, gb18030, cp936 for Windows. The analyst trigger is file-based (ANALYST_RUN_REQUEST) because the cockpit runs inside the app container and cannot start host-side cron containers.

## Related

- produces [[cockpit-dashboard]] — Serves the REST endpoints and state files that app.js renders.
- uses [[directive-writer]] — Routes all human-directive.md writes through scripts/core/directive_writer.py to enforce the PENDING gate and body immutability.
