...
# 4) MCP config discovery. CORRECTED 2026-07-31 — this comment used to claim jcode
#    reads Claude Code's project .mcp.json (citing mcp/protocol.rs load order). It
#    does NOT, in v0.64.2: measured by running a cycle with /app/.mcp.json present and
#    NO ~/.jcode/mcp.json — the model answered NO_MCP. The claim mattered: if it were
#    true, the community Linear server would load alongside the hosted one and bring
#    back the OpenAI invalid_function_parameters kill that commit 2d14485 fixed.
#    The global config is generated by scripts/core/jcode-mcp-config.py. Below we only
#    verify the project file PARSES and registers the
