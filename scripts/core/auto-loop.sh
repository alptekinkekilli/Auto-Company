...
# OPREQ-TOOLPREFIX-TRIM (2026-08-27, panel 1612961e): 15 unused browseros + 2 context7 tools
...
REQUIRED_PROMPT_GUARDRAILS=(
    ...
    # (2026-08-24 Wowcar re-charter: the tender-track gate anchor was retired with
    # its section — the whole block now lives in docs/tender-archive/. The sponsor
    # gate above is the successor brake whose silent loss this list must make loud.)
...
while true; do
    ...
    if [ "${DISCOVERY_ENABLED:-0}" = "1" ]; then
        ...
        _discovery_line="6. Opportunity Discovery is DISABLED (cockpit setting, default) — do NOT scan/rank/propose candidate axes of any kind. The company's whole job is the Wowcar 2.0 program (PROMPT.md \`## WOWCAR 2.0 — STANDING FOCUS\`): the current gate's evidence work, the weekly report, and Human Directive / OPREQ handling."
    fi
...
6. OUTPUT HYGIENE — every tool round-trip re-bills your ENTIRE context. Never stream long command output into the conversation: redirect to a file (\`cmd > /tmp/out.log 2>&1\`) and read back only \`tail -20\`/a grep excerpt. Never dump large files whole (archived memories, PDFs, workbook extracts under \`projects/wowcar/\`) — use grep/offset reads for the rows you need.
...
9. SITE EVIDENCE IS RENDERED EVIDENCE — a \`curl\`/WebFetch of a modern site is NEVER sufficient to conclude that something is ABSENT from it (JS-shell pages render empty to curl; this misclassified a real page once). Before ANY negative claim about a page's content — a mevzuat text, an official announcement, a fee schedule — render it through BrowserOS first. If the render fails, the verdict is INCONCLUSIVE, never negative. Operator instruction 2026-08-01 stands: *“ben browser os boşuna mı kurdum — oradan bakacaksınız.”*
10. STATE RITUAL = ONE CALL, ALREADY MADE — the \`## State Snapshot\` section below is the output of \`scripts/ops/state-snapshot.py\`, pre-run for you at cycle start: directive Status+sha16, open OPREQs, the program-auditor report hash, the Wowcar source-set hash, the operator-decisions hash, and a DELTA line versus the previous cycle. Do NOT re-probe any of those surfaces (no OPREQ grep, no directive re-hash, no manual re-hashing of \`projects/wowcar/\`) unless the snapshot prints ERROR for that field or its DELTA names it as changed. \`DELTA: none\` means the world has not moved: skip ALL re-verification of previous cycles' work (no re-running test suites “as a baseline”, no re-reading code that consensus already records as fixed) and go straight to this cycle's ONE milestone — or the standing mode's one-line empty-cycle confirm. The directive file itself stays canonical — read it when its sha changed or when consensus does not carry its orders.
</rules>
