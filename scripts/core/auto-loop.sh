...
8. NARROW READS — never pull a whole Airtable table. Always scope the query: \`filterByFormula\` for the rows, \`fields[]\` for the columns, \`maxRecords\` as a ceiling. Measured 2026-08-01: \`list_records_for_table\` averaged 29.5 KB per call and its context re-reads cost \$2.38 — more than ALL web research in the same 7 cycles (\$0.34). The same five Qualified rows read with a scoped query are 1.2 KB. This applies to every tool that can return a whole collection, not just Airtable. And fetch LATE: a large result at turn 5 of a 40-turn cycle is re-billed 35 times; the identical result at turn 35 is re-billed 5 times.
$_discovery_line
