...
# $1 = pending count, $2 = attribution-held count, $3.. = extra args
run_watch() {
    local pend="$1" held="$2"; shift 2
    PEND_N="$pend" HELD_N="$held" APP_DIR="$WORK" WATCHER_PATH="$WATCHER" \
    python3 - "$@" <<'PY'
...
def fake_urlopen(req, timeout=None):
    ...
    if "tblREW6MtTMTP5h5N" in url:
        body = {"records": pending if "PENDING" in url else pending}
    else:
        body = {"records": held}
...
out=$(run_watch 1 0 --dry-run 2>&1)
contains "counts reported" "$out" "bridge_pending=1"
...
out=$(run_watch 3 0 --dry-run 2>&1)
contains "fires"              "$out" "would notify"
contains "MERSIS headline"    "$out" "MERSİS oturumu zamanı"
contains "lists a request"    "$out" "REGBR-T-0"
contains "names the ask"      "$out" "MERSİS turu yapalım"

echo "--- 3: empty queue + many attribution holds — COMPANY gap, not operator gap ---"
out=$(run_watch 0 6 --dry-run 2>&1)
contains "fires"                    "$out" "would notify"
...
not_contains "does not demand captchas" "$out" "MERSİS turu yapalım"

echo "--- 4: cooldown — a second call inside repeat-hours stays silent ---"
rm -f "$WORK/logs/.registry-queue-state.json"
out=$(run_watch 3 0 2>&1)                    # real run: writes state (notify is a no-op, no telegram-notify.sh)
contains "first call notifies" "$out" "notified"
out=$(run_watch 3 0 2>&1)
